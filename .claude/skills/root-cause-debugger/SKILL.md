...
- **Parallelize the investigation, never the fix.** Once the cause is confirmed, a single agent applies the smallest safe fix and re-runs the reproduction. During an investigation the main thread reads no source and applies no fix itself — it holds the hypothesis list, the evidence verdicts, and the diagnosis (context-lean-orchestrator).
- **A bug that needs no investigation is not this skill's shape.** When the cause is already known and the fix is one obvious bounded edit — a typo, an inverted condition you can see, a wrong constant — apply it directly (context-lean-orchestrator's Direct mode) and send one verifier to re-run the repro and sweep the neighbors. Reserve the machinery above for causes that are actually unknown; "I'd have to look into it" is the line.
- **Tier by how much judgment the step needs** (operating-discipline §8): an evidence gatherer told exactly what to read or run ("report what `orderService.js:120-180` does with a null `tenantId`", "run this command and paste the exit code and last 20 lines") is `haiku`; a mechanical fix applied to a confirmed cause is `opus`. Diagnosis — weighing evidence and naming the cause — stays at the session default and is never cheapened, because a wrong diagnosis gets believed and built on.
