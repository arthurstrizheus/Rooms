...
Record each answer and each structural choice (layout, chosen layers) as a one-line DECISION with its reason (operating-discipline section 5), in the delegation brief the scaffolding agent works from. A layer the user did NOT choose is recorded as forbidden ("do NOT add"), not merely left unchosen.
...
- AI usage guidance (match model/effort to task complexity — copy the tier table of operating-discipline §8: mechanical no-discovery work on `haiku`, one-surface specced work on `sonnet`, contract implementation on `opus`, design/critique/gates on the session default)
