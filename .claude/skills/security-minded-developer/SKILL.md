...
16. **This domain is on the never-downgrade list** (operating-discipline §8): finders, refuters, fixers, and the probe gate for anything touching auth, permissions, tenancy, secrets, uploads, or broadcasts run at the session default model, or Opus 5 (`opus`) — never `fable`, which is not dispatched for security work either. Only genuinely mechanical support work — "read this middleware and report which routes mount it", "list every route file missing the permission import" — may go to a cheap tier, and its output is evidence to check, never a verdict.
