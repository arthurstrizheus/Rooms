...
- **Multi-area implementation gets ownership-partitioned builder agents** (each owns its files; none touches another's) per context-lean-orchestrator. Single-area feature work is still delegated — to ONE agent rather than a fleet. The exception is trivial work (context-lean-orchestrator's Direct mode): a bounded edit with no discovery and no design judgment — a label, a constant, one page action — is made in the main thread immediately, with one cheap verifier sent afterwards to sweep consumers and run the narrow check. Everything with discovery in it is still an agent's job.
- **Match the model tier to the piece** (operating-discipline §8): mappers and "read this and report" agents go `haiku`; a scoped single-surface change with the pattern already named — add a field to an existing form, mirror a sibling route, add a locale key — goes `sonnet`; builders implementing against the contract go `opus`. Keep the session default for the design and the final gate.
