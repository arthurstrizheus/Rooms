...
- **Single-file cleanups get ONE agent, not a fleet** — and not the main thread, because a cleanup is judgment about what is load-bearing, which is discovery by another name. Delegation is the default for any real cleanup (context-lean-orchestrator); the size of the job picks the fan-out and the tier. Only a trivial tidy with nothing to weigh — deleting an import you can see is unused, fixing one name — is direct-mode work: apply it, then verify.
- **Tier the workers** (operating-discipline §8): a sweep whose pattern is fully stated — rename this symbol everywhere, delete this dead export, apply this import ordering — is `haiku`/`sonnet` per-directory work; restructuring that requires judgment about what is load-bearing is `opus`. The integrator running the behavior-preservation gate stays at the session default.
