...
14. **Model tier for the agents building it** (operating-discipline §8 — separate from the model the feature calls at runtime): inventorying existing AI call sites, adding a prompt/config constant, or writing the status labels is `haiku`; a worker or endpoint that mirrors an existing AI job is `sonnet`; the parsing/validation, failure-and-review, and cost-bounding logic is `opus` or the session default. The go/no-go analysis itself is never cheapened.
