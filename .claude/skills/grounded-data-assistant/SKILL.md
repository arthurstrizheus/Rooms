...
- **Model tier for the agents doing the work** (operating-discipline §8, not to be confused with the model the assistant itself calls at runtime): writing one more read-only tool that mirrors an existing one, or summarizing an existing tool inventory, is `sonnet`/`haiku`; the runner loop, the tenant-scoping design, and the self-verification pass are built at `opus` or the session default.
