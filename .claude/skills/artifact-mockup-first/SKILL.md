...
- **Model tier** (operating-discipline §8): first-pass mockup construction and any redesign that invents layout is `opus` or the session default — design judgment is the product. Iteration once the design is settled — change this copy, swap this color token, add this row to the sample data, republish — is `haiku`/`sonnet` with the exact edit named in the brief.
