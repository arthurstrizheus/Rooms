...
- **Most upkeep here is trivial work — do it directly** (context-lean-orchestrator's Direct mode): retargeting a `data-tour` value to a renamed element, fixing one doc line, or adding tour/doc strings to `en.json` + `es.json` is an inline edit followed by one cheap verifier that greps the tour/doc registry, the selectors, and the locale files for consumers. When such upkeep is a *step inside a larger task*, it is a `haiku` dispatch (operating-discipline §8) — name the exact files and the exact text in the brief. Reporting which registry entries reference a route is `haiku` either way, since it is a read. Authoring a new tour's step sequence or a new help section from scratch is `sonnet`/`opus`. The end-to-end tour run and the §6 critique of new user-facing copy stay at the session default.
