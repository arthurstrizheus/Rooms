...
3. **Update after working.** A change without its `plan.md` update is incomplete. In multi-agent work, workers do NOT edit `plan.md` — it is a shared hot file (context-lean-orchestrator): each worker puts its mapping lines (the After-Working shape below) in `notes_for_integrator`, and the integrator applies all `plan.md` updates in one edit. Doc upkeep on real work is delegated like everything else (context-lean-orchestrator) and it is cheap-tier work: dispatch `haiku` (operating-discipline §8) with the section names and the exact lines to write supplied in the brief — the main thread does not edit `plan.md` during orchestrated work, and a doc agent that has to go discover what changed is the wrong tier and the wrong brief.
   - **Proportional on trivial changes** (context-lean-orchestrator's Direct mode): update what the change actually made stale and nothing else. Most trivial edits touch no `plan.md` claim, and then no update is owed — rule 4 governs, not a reflex to write something. When a doc line *does* name what you changed (a page action, a label users are told to click, a documented default), fix that line in the same direct pass; the verifier agent's sweep covers `plan.md`, `docs/`, and the in-app docs source alongside the code consumers, so staleness is caught rather than assumed away.
