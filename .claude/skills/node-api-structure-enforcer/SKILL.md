...
22. **Tier each dispatch** (operating-discipline §8): reading a route/controller and reporting its shape, listing every caller of an endpoint from a supplied grep, or adding a field to an existing response serializer is `haiku`; a route + controller pair that mirrors a named sibling resource is `sonnet`; a resource with real query, transaction, or permission design is `opus`. Permission wiring, tenancy scoping, and the combined gate stay at the session default — never downgraded.
