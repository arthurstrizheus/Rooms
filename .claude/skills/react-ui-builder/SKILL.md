...
27. Component fleets follow context-lean-orchestrator: parallel builders own disjoint component files — no two agents edit the same file. Every brief uses the delegation-brief template of operating-discipline §8, including its model tier.
28. **Trivial UI edits are done directly, not dispatched** (context-lean-orchestrator's Direct mode): changing a label or copy string, swapping an icon, adding a translation key to `en.json` + `es.json`, toggling or reordering one existing element, or adding / removing / renaming a single page action in a component you can name. Edit it, then send one `haiku` verifier to sweep the consumers — the action's handler, the permission or menu registry entry, the locale keys, the tests and docs that name it — and run the narrow check. No mapping pass, no component inventory, no fleet.
29. **Cheap tiers for the rest of the small work** (operating-discipline §8): reading a component to report its props, or applying a stated change across several named files, is a `haiku` dispatch with exact paths in the brief. Building one specified component or adding a column to an existing table is `sonnet`. Reserve `opus` for builds spanning components, state, or data wiring, and the session default for the design decision and the visual/verification gate.
30. Registry/barrel wiring and shared-file merges — locale dictionaries above all — go through a single integrator, never parallel edits.
31. Shared hot files use context-lean-orchestrator's fragment-staging rule: workers write per-agent fragment files (never the shared file) with safe defaults so the tree stays green; one integrator composes the real file, fails the merge on any key-parity mismatch, then deletes the fragments.
