...
export default function Users({ setLoading }) {
  ...
  const handleSubmit = () => {
    const remove = async () => {
      ...
      await Promise.all(promises).then((resp) =>
        resp
          ? showSuccess(`User${filteredUsers?.length > 1 ? "s" : ""} Deleted`)
          : showError(
              `Failed to delete user${filteredUsers?.length > 1 ? "s" : ""}`
            )
      );
      setSelected([]);
    ...
    const activate = async () => {
      ...
      await Promise.all(promises).then((resp) =>
        resp
          ? showSuccess(`User${filteredUsers?.length > 1 ? "s" : ""} Activated`)
          : showError(
              `Failed to activate user${filteredUsers?.length > 1 ? "s" : ""}`
            )
      );
      setSelected([]);
      setUpdate((prev) => prev + 1);
    ...
    const deactivate = async () => {
      ...
      await Promise.all(promises).then((resp) =>
        resp
          ...
          : showError(
              `Failed to deactivate user${filteredUsers?.length > 1 ? "s" : ""}`
            )
      );
      setSelected([]);
      setUpdate((prev) => prev + 1);
  ...
  useEffect(() => {
    ...
    const data = usrs?.map((itm) => {
      ...
      Usersgroups?.map((gp) =>
        usersGroupsByName.push(groups?.find((mg) => mg.id == gp.group_id))
      );
      return createData(
        itm.id,
