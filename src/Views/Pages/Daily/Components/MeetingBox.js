...
const MeetingBox = ({
  ...
  useEffect(() => {
    ...
    if (
      ((start.getHours() === end.getHours() &&
        start.getMinutes() + 15 === end.getMinutes()) ||
        (start.getHours() + 1 === end.getHours() &&
          start.getMinutes() + 15 === 60 &&
          end.getMinutes() === 0)) &&
      meeting?.type
    ) {
      setPadLeft(false);
  ...
  const getColorByType = () =>
    meetingTypesRes?.find((mt) => mt.id == meeting.type).color;
      ...
      title={
        <>
          <Grid textAlign={"center"}>
            <Typography fontSize={15} fontFamily={"Candara"}>
              {meeting.name}
            </Typography>
            <Typography fontSize={13} variant="caption">
              {getHours(meeting.start_time)}:
              {String(start.getMinutes()).padStart(2, "0")}
              {getAmPm(start)}m - {getHours(meeting.end_time)}:
