...
const DayCalender = ({
  ...
  useEffect(() => {
    ...
    if (hoursScrollRef.current) {
      hoursScrollRef.current.addEventListener(
        "wheel",
    ...
    if (Cref.current) {
      Cref.current.addEventListener("wheel", preventHorizontalScroll, {
        passive: false,
      });
    }
    Cref2.current.forEach((ref) => {
      if (ref) {
        ref.addEventListener("wheel", preventHorizontalScroll, {
          passive: false,
        });
      }
    });
    ...
    return () => {
      if (hoursScrollRef.current) {
        hoursScrollRef.current.removeEventListener(
          "wheel",
          preventHorizontalScroll
        );
      }
      if (Cref.current) {
        Cref.current.removeEventListener("wheel", preventHorizontalScroll);
      }
      Cref2.current.forEach((ref) => {
        if (ref) {
          ref.removeEventListener("wheel", preventHorizontalScroll);
        }
      });
    };
  }, [hoursScrollRef, Cref, Cref2]);
  ...
  const getMeeting = (room) => {
    let roomMeetings = filteredMeetings?.filter(
      (meet) => meet?.room == room || meet?.room_id == room
    );
