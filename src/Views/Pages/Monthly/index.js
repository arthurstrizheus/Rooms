...
const MonthSchedulePage = ({ setLoading, selectedDate, setSelectedDate }) => {
    const [roomsObj, setRoomsObj] = useState([]);
    const [rooms, setRooms] = useState([]);
    const [locations, setLocations] = useState([]);
    const [meetings, setMeetings] = useState([]);
    const [meetingTypes, setMeetingTypes] = useState([]);
    const [roomsWidth, setRoomsWidth] = useState(220);
    ...
    useEffect(() => {
        const data = async () => {
            ...
            const mts = await GetMeetingsByUserId(user?.id, {date: startOfMonth(selectedDate), range:'Month'});
