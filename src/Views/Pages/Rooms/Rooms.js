...
export default function Rooms({ setLoading }) {
    ...
    const handleDeleteSelected = () => {
        const remove = async () => {
            const promises = filteredRooms?.map(async (itm) =>
                isSelected(itm.id) ? await DeleteRoom(itm.id) : null
            );
                    ...
                    <Table sx={{ minWidth: 700 }} aria-label="customized table">
                            ...
                            <TableRow>
                                ...
                                <StyledTableCell align="left">
                                    <TableSortLabel
                                        ...
                                        direction={
                                            orderBy === "color" ? order : "asc"
                                        }
                                        onClick={(event) =>
                                            handleRequestSort(event, "color")
                                        }
                                    >
                                        Color
                                    </TableSortLabel>
                        ...
                        <TableBody sx={{ backgroundColor: "white" }}>
                            {paginatedRows?.map((row, index) => {
                                ...
                                const roomGroup = roomGroups.filter(
                                    (rgrp) => rgrp.room_id === row.id
                                );
                                const roomGroupIds = roomGroup.map(
                                    (rg) => rg.group_id
                                );
                                ...
                                const rowRoom = filteredRooms?.find(
                                    (fr) => fr.id === row.id
                                );
                                return (
