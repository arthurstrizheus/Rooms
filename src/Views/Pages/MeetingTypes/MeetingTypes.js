...
export default function MeetingTypes({ setLoading }) {
  ...
  const handleDeleteSelected = () => {
    const remove = async () => {
      const promises = types?.map(async (itm) =>
        isSelected(itm.id) ? await DeleteMeetingType(itm.id) : null
      );
            ...
            <TableHead sx={{ position: "sticky", top: 0, zIndex: 1 }}>
              <TableRow>
                <StyledTableCell padding="checkbox">
                  <Checkbox
                    indeterminate={
                      selected.length > 0 && selected.length < types.length
                    }
                    checked={
                      types.length > 0 && selected.length === types.length
                    }
