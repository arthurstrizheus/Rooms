...
export default function BlockedDates({ setLoading }) {
  ...
  const handleDeleteSelected = () => {
    const remove = async () => {
      const promises = filteredDates?.map(async (itm) =>
        isSelected(itm.id) ? await DeleteBlockedDate(itm.id) : null
      );
  ...
  useEffect(() => {
    if (blockedDates?.length) {
      const filteredBlockedDates = blockedDates?.filter((bd) =>
        rooms?.find(
          (fr) => fr.id == bd.room_id && fr.location === filterLocation.officeid
        )
      );
          ...
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            ...
            <TableBody sx={{ backgroundColor: "white" }}>
              {paginatedRows?.map((row) => {
                ...
                const endString = new Date(row.end_time).toLocaleDateString(
                  "en-US",
                  {
                    weekday: "long",
                    month: "short",
                  ...
                  <React.Fragment key={row.id}>
                    ...
                    >
                      ...
                      <StyledTableCell align="left">{`${
                        ...
                        2,
                        "0"
                      )}${getDateAmPm(start)}m`}</StyledTableCell>
                      <StyledTableCell align="left">{`${
                        end.getHours() % 12 ? end.getHours() % 12 : 12
                      }:${String(end.getMinutes()).padStart(
                        2,
                        "0"
                      )}${getDateAmPm(end)}m`}</StyledTableCell>
                      <StyledTableCell align="left">
                        {row.repeats ? row.repeats : "No"}
                      </StyledTableCell>
                    </StyledTableRow>
                  </React.Fragment>
                );
