const { MeetingGroup } = require('../models');

const GetAll = async (req, res) => {
    try {
        const data = await MeetingGroup.findAll();
        res.json(data);
    } catch (err) {
...
const Post = async (req, res) => {
    try {
        ...
        const { group_id, meeting_id, created_user_id } = req.body;

        // Validate the incoming data (optional but recommended)
        if (!group_id || !meeting_id || !created_user_id) {
        ...
        const newResource = await MeetingGroup.create({
            group_id,
            meeting_id,
            created_user_id,
        });
...
const Update = async (req, res) => {
    try {
        ...
        const { group_id, meeting_id, created_user_id } = req.body;  // Extract data from the request body
        ...
        const resource = await MeetingGroup.findByPk(id);
        if (!resource) {
