...
const IsParentMeeting = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await MeetingRecurrence.findAll({
            where: { meeting_id: id },
        });
        if (data?.length) {
            return res.status(200).json({
                message: "Meeting is parent recurring meeting",
                parent: true,
            });
        } else {
            return res.status(200).json({
                message: "Meeting is NOT parent recurring meeting",
                parent: false,
            });
        }
    } catch (err) {
...
const CancelAllMeetings = async (req, res) => {
    ...
    try {
        ...
        if (Number(id) === -1) {
            ...
            canDelete = await meetingController.CanDelete(
                parentMeeting.id,
                req.user?.id
            );
        } else {
            canDelete = await meetingController.CanDelete(id, req.user?.id);
        }
