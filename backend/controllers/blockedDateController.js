...
const { BlockedDate, Room } = require("../models");
...
const Post = async (req, res) => {
    try {
        ...
        if (room.location !== req.user.location) {
            return res.status(403).json({
                message:
                    "You do not have permission to block dates for this room",
            });
        }
        ...
        if (!room_id || !start_time || !name || !end_time || !created_user_id) {
            return res.status(400).json({
                message:
                    "room_id, start_time, description, name, end_time, repeats, and created_user_id are required",
            });
        }
...
const Delete = async (req, res) => {
    try {
        ...
        const room = await Room.findByPk(resource.room_id);
        if (room.location !== req.user.location) {
            return res.status(403).json({
                message:
                    "You do not have permission to delete block dates for this room",
            });
        }
