const { RoomGroup, Room } = require("../models");
    ...
    try {
        ...
        if (!group_id || !room_id || !created_user_id) {
            return res.status(400).json({
                message: "group_id, room_id, and created_user_id are required",
            });
        }
        ...
        if (req.user?.office_admin != room.location && !req.user?.admin) {
            return res.status(403).json({
                message: "Cannot modify room permissions for another office.",
            });
        }
...
const Delete = async (req, res) => {
    try {
        ...
        if (req.user?.office_admin != room.location && !req.user?.admin) {
            return res.status(403).json({
                message: "Cannot delete room permissions for another office.",
            });
        }
...
const DeleteRoomId = async (req, res) => {
    try {
        ...
        if (!group_id || !room_id) {
            return res
                .status(400)
                .json({ message: "group_id and room_id are required" });
        }
