...
async function GetNextParentMeeting(userId, meeting) {
  ...
  fakeMeets?.map((fm) =>
    console.log({
      start: fm.start_time,
      equal: fm.start_time === nextDate.toISOString(),
    })
  );
...
async function CreateRepeatingMeetingsOfThisMeeting(
  meeting,
  userId,
  currentDate
) {
...
const UpdateAllRecurrence = async (req, res) => {
  try {
    ...
    fakeMeets = fakeMeets.filter(
      (fm) =>
        new Date(fm.start_time) >= new Date(start_time) ||
        fm.recurrence_id != recurrence_id
    );
...
const UpdateAllNextInRecurrence = async (req, res) => {
  try {
    ...
    let fakeMeets = await CreateRepeatingMeetings(
      start_time,
      "Month",
      created_user_id
    );
