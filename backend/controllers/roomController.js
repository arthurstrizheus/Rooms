...
const GetRoomsUserCanSee = async (req, res, next) => {
  try {
    ...
    console.log("Fetching rooms for user:", userId); // Debugging log
    ...
    if (user.admin) {
      const rooms = await Room.findAll({ where: { location: user.location } });
  ...
  try {
    ...
    if (!value || !color || !created_user_id) {
      return res.status(400).json({
        message: "Required fields missing: value, color, created_user_id",
      });
    }
